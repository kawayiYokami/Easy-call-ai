    ...
    let (block_name, body) = match tool_id.trim() {
        ...
        "exec" => (
            ...
            "何时必须用：当必须通过命令、程序或脚本来搜索文件、读取信息、检查环境、运行验证或执行脚本时，使用 exec。很多 skill 会要求执行脚本。\n\
             ...
             如何使用：优先把 exec 用于搜索、读取、检查、运行和验证；exec run 默认是一次性进程执行，命令结束、失败或超时后会回收本次进程树，不依赖持久 live shell 状态。执行前先判断是否存在更低风险替代，并尽量缩小命令影响范围。搜索文件或文本时，若环境可用，优先使用 rg / rg --files；缺失时再使用当前 shell 的原生命令。读取文件、列目录、搜索文本等互不依赖的探索命令应尽量合并为少量命令，避免反复启动无关命令。运行测试或构建时，只执行和本次改动直接相关的最小必要检查。若任务是新增或修改文件，应先使用 apply_patch；若 apply_patch 失败，应先修正补丁格式或路径问题重试，而不是改用 exec 通过 cat、echo、重定向、heredoc 等方式写文件。不要执行破坏性命令（删除、移动、清空、重置仓库等），除非用户明确要求。Windows 下不要跨 shell 拼接破坏性文件操作；如必须处理路径，优先使用当前 shell 的原生命令和明确路径。若 exec 报告工作区、路径授权或 shell_switch_workspace 相关错误，应先解决工作区与授权前提，再继续当前工具链，不要在 exec 与 apply_patch 之间来回试错。\n\
