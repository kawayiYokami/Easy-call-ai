...
fn operate_provider_tool_definition() -> ProviderToolDefinition {
    ProviderToolDefinition::new(
        OPERATE_TOOL_NAME,
        "统一桌面脚本工具。入参只有 script:string，一行一个动作。\n可用语法：\nmouse <button> click @x,y [repeat=n] [delay=s] [pre_delay=s] [press=s]\nmouse scroll_up [repeat=n] [delay=s] [pre_delay=s]\nmouse scroll_down [repeat=n] [delay=s] [pre_delay=s]\nmouse move @x,y [pre_delay=s]\napp <windowId> click el=<n>|@x,y [repeat=n] [dblclick=true] [pre_delay=s] [post_delay=s]\napp <windowId> setvalue el=<n> \"内容\" [pre_delay=s] [post_delay=s]\napp <windowId> getvalue el=<n>\napp <windowId> scroll_up el=<n>|@x,y [repeat=n] [delay=s] [pre_delay=s] [post_delay=s]\napp <windowId> scroll_down el=<n>|@x,y [repeat=n] [delay=s] [pre_delay=s] [post_delay=s]\napp <windowId> key <combo> [repeat=n] [delay=s] [pre_delay=s] [post_delay=s]\nkey <combo> [repeat=n] [delay=s] [pre_delay=s] [press=s]\ntext \"内容\" [repeat=n] [delay=s] [pre_delay=s]\nwait <seconds>\nscreenshot [focused_window|window_id=<id>] [region=@x,y,w,h] [elements=true] [save=\"绝对路径\"] [quality=1..100]\n参数说明：button=left|right|middle|back|forward；combo 用 + 连接按键，如 Control+L、Control+Shift+P、Enter；x/y/w/h 为 0~1 百分比坐标；repeat=1~100；delay/pre_delay/post_delay/press=0~300 秒；save 必须是绝对路径；quality 默认 75；windowId 与 window_id= 支持十进制或 0x 前缀十六进制（与控件树返回的 windowId 一致）。\napp 动作是对指定窗口的后台操作：不抢焦点、不移动真实光标，仅 Windows 可用。el=<n> 引用最近一次 screenshot（需 elements=true）返回的 ref 编号；app 步骤结果的 method 字段报告实际投递方式（invoke/valuepattern/scrollpattern/postmessage）。元素树在截图后发生变化时动作会明确报错（附 el 与变化摘要：原类型/名称 vs 现类型/名称），需重新截图。setvalue 为整体替换文本控件内容。getvalue 通过 ValuePattern 读回文本控件当前值（写后读回可验证写入结果，无需截图）；控件不支持 ValuePattern 时明确报错。click 的 dblclick=true 走后台双击（仅 postmessage 路径，invoke-only 控件无效）；app 动作结果的 focus 字段（focus=Type('name')）报告动作后目标窗口内部焦点控件去向，setvalue/key 后用它确认焦点位置，不必重截。post_delay 在动作完成后等待 UI 响应（如联想词、弹出菜单）再返回，避免下一步拿到过期元素树。app key 把按键投递到目标窗口的内部焦点控件（适用于回车提交、Esc 关弹窗、Ctrl+A 全选等），但全局快捷键/菜单加速键类按键可能不响应。部分应用（UWP/WebView2 等）不响应 postmessage 兜底，只能依赖 UIA pattern。\n规则：screenshot 对模型只保留最新一张，旧画面视为已经离去。\n需要确认准确点击坐标时，用局部截图（region=@x,y,w,h、window_id=<id> 或 focused_window）配合 elements=true，latest_screenshot.tree 会返回该区域内的可交互控件列表（windowId/windowTitle/controlType/name/x/y/width/height/ref，坐标为相对主屏的 0~1 归一化、与 mouse 的 @x,y 同坐标系；区域外控件已过滤，不会返回）；只查看画面、不需要点击时不要带 elements=true，避免多余的控件扫描开销。focused_window 只返回聚焦窗口的控件，window_id=<id> 截取并扫描指定后台窗口，region 只返回区域内的控件，desktop 返回全部可见窗口的控件（元素多，谨慎使用）。控件树平台说明：Windows 用系统 UI Automation；Linux（X11）用 AT-SPI2，Wayland 或未启用辅助功能时返回空；macOS 用 AXUIElement，需在系统设置中授予辅助功能权限。",
        serde_json::json!({
            "type": "object",
            "properties": {
                "script": {
