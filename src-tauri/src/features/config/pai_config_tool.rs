...
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
struct ConfigWorkspaceLoadError {
    item: String,
    error: String,
...
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
struct ConfigSkillListResult {
    skills: Vec<ConfigSkillSummaryItem>,
    errors: Vec<ConfigWorkspaceLoadError>,
...
fn handle_skill(ctx: &CliContext, args: &[String]) -> Result<(), String> {
    ...
    match cmd {
        "ls" => {
            let (skills, errors) = load_config_skill_summaries_with_errors(ctx)?;
            let skill_summary = render_config_skill_summary(&skills);
            print_json(&ConfigSkillListResult {
                skills,
                errors,
                skill_summary,
            })
        ...
        "get" => {
            let selector = required_arg(args, 1, "skill get <name-or-id>")?;
            let skill_dir = find_skill_dir(ctx, selector)?;
            let skill_md = skill_dir.join("SKILL.md");
            let content = fs::read_to_string(&skill_md)
                .map_err(|err| format!("读取 SKILL.md 失败 ({}): {err}", skill_md.display()))?;
            print_json(&serde_json::json!({
                "name": skill_dir.file_name().and_then(|v| v.to_str()).unwrap_or_default(),
                "path": skill_dir.to_string_lossy().to_string(),
                "content": content
            }))
        }
        "example" => {
            push_output(&skill_example());
            Ok(())
        }
        "new" => {
            let name = required_arg(args, 1, "skill new <name>")?;
            push_output(&build_skill_template(name));
            Ok(())
        }
        "export" => {
            let selector = required_arg(args, 1, "skill export <name-or-id> <dir>")?;
            let dir = required_arg(args, 2, "skill export <name-or-id> <dir>")?;
            let skill_dir = find_skill_dir(ctx, selector)?;
            copy_dir_recursive(&skill_dir, Path::new(dir))?;
            print_output_path(dir)
        }
        "check" => {
            let dir = required_arg(args, 1, "skill check <dir>")?;
            validate_skill_dir(Path::new(dir))?;
            print_json(&serde_json::json!({"ok": true, "path": dir}))
        }
        "diff" => {
            let selector = required_arg(args, 1, "skill diff <name-or-id> <dir>")?;
            let dir = required_arg(args, 2, "skill diff <name-or-id> <dir>")?;
            let current_dir = find_skill_dir(ctx, selector)?;
            validate_skill_dir(Path::new(dir))?;
            let current = fs::read_to_string(current_dir.join("SKILL.md"))
                .map_err(|err| format!("读取当前 skill 失败 ({}): {err}", current_dir.display()))?;
            let next = fs::read_to_string(Path::new(dir).join("SKILL.md"))
                .map_err(|err| format!("读取目标 skill 失败 ({}): {err}", Path::new(dir).display()))?;
            print_json(&serde_json::json!({
                "planSummary": if current == next { "0 个文件将变化" } else { "1 个文件将变化" },
                "planItems": if current == next { Vec::<String>::new() } else { vec!["SKILL.md".to_string()] },
                "changedFields": if current == next { Vec::<String>::new() } else { vec!["SKILL.md".to_string()] },
                "affectedResources": [current_dir.to_string_lossy().to_string()]
            }))
        }
        "update" => {
            let selector = required_arg(args, 1, "skill update <name-or-id> <dir>")?;
            let dir = required_arg(args, 2, "skill update <name-or-id> <dir>")?;
            validate_skill_dir(Path::new(dir))?;
            let target = find_skill_dir(ctx, selector)?;
            copy_dir_recursive(Path::new(dir), &target)?;
            print_json(&serde_json::json!({"ok": true, "path": target.to_string_lossy().to_string()}))
        }
        "delete" => {
            let selector = required_arg(args, 1, "skill delete <name-or-id> --confirmed")?;
            require_delete_confirmed(args, "skill delete <name-or-id> --confirmed")?;
            let target = find_skill_dir(ctx, selector)?;
            delete_skill_dir(ctx, &target)?;
            print_json(&serde_json::json!({"ok": true, "path": target.to_string_lossy().to_string()}))
        }
        _ => Err("用法: skill ls|get|example|new|export|check|diff|update|delete".to_string()),
    }
}
...
fn apply_department_tree(departments: &mut [DepartmentConfig], tree: &DepartmentTreeFile) -> Result<(), String> {
    validate_department_tree(tree)?;
    let known = departments
        .iter()
        .map(|item| item.id.clone())
        .collect::<BTreeSet<_>>();
    for node in &tree.departments {
        if !known.contains(&node.id) {
            return Err(format!("tree 包含未知部门: {}", node.id));
        }
        if let Some(parent_id) = &node.parent_id {
            if !known.contains(parent_id) {
                return Err(format!("tree 包含未知父部门: {parent_id}"));
            }
        }
    }
...
fn list_skill_names(ctx: &CliContext) -> Result<Vec<String>, String> {
    let dir = effective_workspace_root(ctx).join("skills");
    if !dir.exists() {
        return Ok(Vec::new());
    }
    let mut names = fs::read_dir(&dir)
        .map_err(|err| format!("读取 skills 目录失败 ({}): {err}", dir.display()))?
...
fn find_skill_dir(ctx: &CliContext, selector: &str) -> Result<PathBuf, String> {
    let root = effective_workspace_root(ctx).join("skills");
    for name in list_skill_names(ctx)? {
        if name.eq_ignore_ascii_case(selector) {
            return Ok(root.join(name));
        }
    }
    Err(format!("skill not found: {selector}"))
}

fn load_config_skill_summaries_with_errors(
    ctx: &CliContext,
) -> Result<(Vec<ConfigSkillSummaryItem>, Vec<ConfigWorkspaceLoadError>), String> {
    let skills_dir = effective_workspace_root(ctx).join("skills");
    if !skills_dir.exists() {
        return Ok((Vec::new(), Vec::new()));
    }
    let mut dirs = fs::read_dir(&skills_dir)
        .map_err(|err| format!("读取 skills 目录失败 ({}): {err}", skills_dir.display()))?
        .filter_map(|entry| entry.ok().map(|item| item.path()))
        .filter(|path| path.is_dir())
        .collect::<Vec<_>>();
    dirs.sort();
    ...
    for dir in dirs {
        let id = dir
            ...
            .unwrap_or_default()
            .to_string();
        if crate::workspace_preset_skill_dir_name(&id) {
            continue;
        }
        let skill_md = dir.join("SKILL.md");
        if !skill_md.is_file() {
            errors.push(ConfigWorkspaceLoadError::with_hint(
        ...
        match parse_config_skill_file(&skill_md) {
            Ok((name, description, _content)) => skills.push(ConfigSkillSummaryItem {
                id,
                name,
                description,
                path: skill_md.to_string_lossy().to_string(),
            }),
            Err(err) => errors.push(ConfigWorkspaceLoadError::with_hint(
                skill_md.to_string_lossy().to_string(),
                err,
                "检查 SKILL.md 的 frontmatter、name/description 与正文格式，修复后重新调用 skill ls。",
            )),
        }
    }
    Ok((skills, errors))
...
fn parse_config_skill_file(skill_md_path: &Path) -> Result<(String, String, String), String> {
    ...
    let first = lines
        ...
        .to_string();
    if first != "---" {
        return Err("SKILL.md 必须以 frontmatter 开头".to_string());
    }
...
fn render_config_skill_summary(skills: &[ConfigSkillSummaryItem]) -> String {
    if skills.is_empty() {
    ...
    skills
        ...
        .map(|item| {
            ...
            format!("- {}: {}", item.name.trim(), desc)
        })
        .collect::<Vec<_>>()
        .join("\n")
}

fn ensure_skill_not_system_preset(target: &Path) -> Result<(), String> {
    let id = target
        .file_name()
        .and_then(|value| value.to_str())
        .unwrap_or_default();
...
fn validate_skill_dir(path: &Path) -> Result<(), String> {
    ...
    if !skill_md.exists() {
        return Err(format!("skill 缺少 SKILL.md: {}", skill_md.display()));
    }
    let content = fs::read_to_string(&skill_md)
        .map_err(|err| format!("读取 SKILL.md 失败 ({}): {err}", skill_md.display()))?;
        ...
        .map_err(|err| format!("读取目录失败 ({}): {err}", from.display()))?
    {
        ...
        if source.is_dir() {
            ...
            fs::copy(&source, &target).map_err(|err| {
                format!("复制文件失败 ({} -> {}): {err}", source.display(), target.display())
            })?;
        }
    }
    Ok(())
...
fn delete_skill_dir(ctx: &CliContext, target: &Path) -> Result<(), String> {
    ensure_skill_not_system_preset(target)?;
    let skills_root = effective_workspace_root(ctx).join("skills");
    let root = skills_root
        .canonicalize()
        .map_err(|err| format!("解析 skills 根目录失败 ({}): {err}", skills_root.display()))?;
    let target = target
        .canonicalize()
        .map_err(|err| format!("解析 skill 目录失败 ({}): {err}", target.display()))?;
    ...
    fs::remove_dir_all(&target)
        .map_err(|err| format!("删除 skill 目录失败 ({}): {err}", target.display()))
}
...
fn skill_example() -> String {
    build_skill_template("example-skill")
}

fn build_skill_template(name: &str) -> String {
    format!(
        "---\nname: {name}\ndescription: 简要说明这个 skill 解决什么问题。\n---\n\n在这里写 skill 正文。"
    )
}
