...
fn default_leader_department(api_config_id: &str) -> DepartmentConfig {
    ...
    DepartmentConfig {
        ...
        guide: [
            ...
            "",
            "`delegate` 的参数必须写清：`department_id` 是目标下级部门 ID；`background` 包含父任务、已知事实、前序子任务结果、关键约束和必要上下文；`question` 明确定义这次子任务要完成或查清什么；`focus` 写明优先关注点、范围边界、交付要求和需要避免的方向；`mode` 固定使用 `wait`，确保你能等待子任务结果并在同一轮对话中继续整合和推进。`wait` 可以并发发出多个委托，它只表示等待结果，不表示串行。",
            "",
