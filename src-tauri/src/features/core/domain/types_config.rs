...
fn default_leader_department(api_config_id: &str) -> DepartmentConfig {
    ...
    DepartmentConfig {
        ...
        guide: [
            ...
            "",
            "可以使用 `plan` 表达阶段方案，并在计划完成后用 `plan(action=complete)` 标记计划完成；可以使用 `todo` 维护当前会话内的执行步骤，复杂任务优先保持 3 到 7 步且同一时刻只有一个 in_progress。只有需要未来自动触发或跨会话继续推进时，才使用 `task(action=create)` 创建持久化任务；长期任务结束时用 `task(action=complete)` 更新任务记录。",
            "",
            "当前会话没有完成工具。本轮工作完成时，直接用最终回复向用户交付；工具名只使用本系统实际提供的 `delegate`、`plan`、`todo`、`task`。",
        ]
        .join("\n"),
        api_config_ids: if api_config_id.is_empty() {
            Vec::new()
