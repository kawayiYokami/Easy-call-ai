      ...
      <div class="card-body gap-4 p-4">
        ...
        <div class="grid gap-3 md:grid-cols-[minmax(0,1fr)_auto] md:items-end">
          ...
          <button
            ...
            :disabled="!portDirty || props.savingConfig"
            @click="savePort"
          >
            <Save class="h-4 w-4" />
        ...
        <div class="grid gap-3">
          <div class="rounded-box border border-base-300 bg-base-200/40 p-3">
            <div class="flex items-center justify-between gap-3">
              ...
              <button class="btn btn-sm btn-primary shrink-0" :disabled="!webInfo?.localUrl" @click="openLocalUrl">
          ...
          <div class="rounded-box border border-base-300 bg-base-200/40 p-3">
            <div class="flex items-center justify-between gap-3">
              <div class="min-w-0">
                ...
                <div class="mt-1 font-mono text-sm">{{ webInfo?.remotePassword || "-" }}</div>
              </div>
              <button class="btn btn-sm btn-ghost shrink-0" :disabled="!webInfo?.remotePassword" @click="copyText(webInfo?.remotePassword || '')">
                <Copy class="h-4 w-4" />
              </button>
            </div>
          </div>
          ...
          <div class="rounded-box border border-base-300 bg-base-200/40 p-3">
            <div class="flex items-center justify-between gap-3">
              ...
              <button class="btn btn-sm btn-ghost shrink-0" :disabled="remoteUrls.length === 0" @click="copyText(remoteUrls.join('\n'))">
                <Copy class="h-4 w-4" />
              </button>
            </div>
            ...
            <div v-else class="mt-2 text-xs text-base-content/60">
              {{ t("config.networkAccess.noRemoteLink") }}
            </div>
          </div>
        </div>
...
import { Copy, ExternalLink, RefreshCw, Save } from "@lucide/vue";
...
const localUrlText = computed(() => webInfo.value?.localUrl || t("config.networkAccess.waiting"));
